...
.metric-box { background:#f8f9fa; border-radius:10px;
              padding:1rem; text-align:center; }
.metric-box .num { font-size:2rem; font-weight:700; }
.metric-box .lbl { font-size:0.8rem; color:#666; }

.rec-item { padding:0.4rem 0.6rem; background:#f1f8f5;
            border-radius:6px; margin-bottom:0.4rem;
            font-size:0.88rem; border-left:3px solid #1D9E75; }
.warn-box  { background:#FFF3CD; border:1px solid #EF9F27;
             border-radius:8px; padding:0.8rem 1rem;
             font-size:0.85rem; color:#633806; margin-top:0.8rem; }
.info-box  { background:#E6F1FB; border:1px solid #378ADD;
             border-radius:8px; padding:0.8rem 1rem;
             font-size:0.85rem; color:#042C53; margin-top:0.8rem; }
...
@st.cache_resource
def get_model():
    df = pd.read_csv(Path(__file__).parent / "train_data.csv")
    return train(df)
...
# ── 모델 로드 ─────────────────────────────────────────────────────────────────
with st.spinner("AI 예측 모델 초기화 중..."):
    pipeline = get_model()

# ── 탭 ───────────────────────────────────────────────────────────────────────
tab_b2b, tab_b2c, tab_stats = st.tabs(["🏢 개발사·플랫폼 도구", "👥 이용자 등급 조회", "📊 통계 대시보드"])
...
with tab_b2b:
    ...
    with col3:
        ...
        description = st.text_area("콘텐츠 기술서", height=118,
                                   placeholder="폭력성·선정성·사행성·언어 관련 내용을 기술하세요.\n예) 총기 전투, 혈액 표현, 확률형 아이템 포함")
    ...
    if run:
        ...
        with c3:
            ...
            if level == "높음":
                ...
                <div class="warn-box">
                ⚠️ <b>사전 검토 권고</b><br>
                등록 전 GRAC 사전 자문 또는<br>GCRB 가이드라인 재검토를 권고합니다.
                </div>""", unsafe_allow_html=True)
            else:
                st.markdown("""
                <div class="info-box">
                ℹ️ 콘텐츠 기술서를 상세히 작성하면<br>예측 정확도가 높아집니다.
                </div>""", unsafe_allow_html=True)
...
with tab_b2c:
    ...
    if search and query:
        ...
        with col_a:
            st.markdown(f"### {query}")
            st.markdown(f"""
            <span class="trust-badge {badge_css}">신뢰도 {level}</span>
            """, unsafe_allow_html=True)
            ge = meta["grade"]
        ...
        with col_c:
            if level == "하":
                ...
                st.warning("🔍 **일부 항목 검토 권고**\n\n등급 부여 기준이 일부 항목에서 재확인이 필요할 수 있습니다.")
            else:
...
with tab_stats:
    ...
    st.caption("※ 아래 통계는 합성 학습 데이터 기반입니다. 실제 GRAC/GCRB 데이터 연계 시 자동 갱신됩니다.")
    ...
    with m1:
        st.metric("전체 분석 건수", f"{total:,}건")
    with m2:
        st.metric("재조정 발생률", f"{reratio:.1%}")
    with m3:
        st.metric("고위험 게임", f"{high_n:,}건", f"전체의 {high_n/total:.1%}")
    with m4:
        st.metric("개인 개발사 비율", f"{ind_n/total:.1%}")
    ...
    with ch1:
        ...
        fig1 = px.bar(genre_df, x="재조정비율", y="장르", orientation="h",
                      ...
                      text=genre_df["재조정비율"].apply(lambda x: f"{x:.0%}"))
    ...
    with ch2:
        ...
        fig2 = px.bar(pivot, x="등급", y="재조정비율", color="기관유형",
                      ...
                      text=pivot["재조정비율"].apply(lambda x: f"{x:.0%}"))
...
# ── 푸터 ─────────────────────────────────────────────────────────────────────
st.markdown("---")
st.caption("GradeChecker v0.1 | 게임이용자보호센터 | 2026 문화 디지털혁신 및 데이터 활용 공모전 출품작 | 본 서비스는 프로토타입이며, 실제 등급 판정 효력이 없습니다.")
